...
public class RandomReversingSubsequence : ILLHFunctionInterface
{ private int counter;
    ...
    {
        return RandomReversingSubsequenceFunction(new List<int>(orgTour));
    }

    private List<int> RandomReversingSubsequenceFunction(List<int> orgTour)
    {
    // Random Reversing of Subsequence (RRS): Đảo ngược ngẫu nhiên một đoạn tour

    // Chọn ngẫu nhiên một đoạn tour từ i đến j.
    // Đảo ngược thứ tự các thành phố trong đoạn đó.        
        List<int> tour = new List<int>(orgTour);
        Random random = new Random();
        int i = random.Next(tour.Count);
        int j = random.Next(tour.Count);
        int start = Math.Min(i, j);
        int end = Math.Max(i, j) + 1; // Adjusted to include the end index
        tour.Reverse(start, end - start);
        return tour;
    }
